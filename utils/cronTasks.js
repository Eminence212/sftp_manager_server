const { User, Customer, Acknowledgement } = require("../models");
...
const getCbsRemoteFiles = async (customer, directory, createdAt) => {
  const inbound_amp = customer?.inbound_amp,
    ...
    erreur_amp = customer?.erreur_amp,
    archive_amp = customer?.archive_amp;
...
const getCbsRemoteAllFiles = async (customer, createdAt) => {
  ...
  for (const key of Object.keys(files)) {
    files = {
      ...files,
      [key]: files[key].filter((item) => isEqual(item.modifyTime, createdAt)),
    };
  }
  return files;
};
        ...
        switch (key) {
          case "out":
            for (let j = 0; j < data.length; j++) {
              ...
              const { state } = await sftp.downloadFile(remotePath, localPath);
              //Sauvegarde du fichier dans la base de données
              const newFile = await Acknowledgement.findOne({
                where: { fileName: data[j].name },
              });
              if (!newFile) {
                await Acknowledgement.create({
                  customerId: customer.id,
                  fileName: data[j].name,
                  fileModifyTime: data[j].modifyTime,
                  fileAccessTime: data[j].accessTime,
                });
              }
              //
            }
...
const getRemoteInfo = async (customer, file_name, directory) => {
  ...
  const outbound_amp = customer?.outbound_amp,
    erreur_amp = customer?.erreur_amp,
    archive_amp = customer?.archive_amp;
...
const putRemoteFiles = async (customer) => {
  ...
  try {
    ...
    console.error("Uploadings failed:", error);
  }
};
        ...
        switch (key) {
          ...
          case "out":
            for (let j = 0; j < data.length; j++) {
              ...
              //Vérifier si le fichier n'était pas encore envoyé ver le compte sftp du client
              const newFile = await Acknowledgement.findOne({
                where: { fileName: data[j].name },
              });

              if (newFile) {
                const { id, sent } = newFile;
                if (!sent) {
                  const { state } = await sftp.uploadFile(
                    localPath,
                    outbound + "/" + data[j].name
                  );
