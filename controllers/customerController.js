...
const userController = {
  ...
  getMonitoring: async (req, res) => {
    ...
    const monitoringDate = createdAt ? new Date(createdAt) : new Date();
    let customersList = [];
    try {
      const customers = await Customer.findAll({
        attributes: [
      ...
      if (customers.length > 0) {
        for (let i = 0; i < customers.length; i++) {
          ...
          const dataValues = customers[i]?.dataValues;
          files = {
            ...files,
            ...(await getCbsRemoteAllFiles(dataValues, monitoringDate)),
          };

          const newCustomer = {
            ...dataValues,
            files: { ...files },
            monitoringDate: formaShorttDate(monitoringDate),
          };
